...
function BlogListContent({ posts, categories }: BlogListClientProps) {
  const { t } = useLocale()
  ...
  // Filter and sort posts（数据由服务端全量传入，含全部已发布文章）
  const filteredPosts = posts
    .filter(post => selectedCategory === 'all' || post.category?.slug === selectedCategory)
    .sort((a, b) => {
      switch (sortBy) {
        case 'oldest':
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div className={styles.filterBar}>
            ...
            <span className={styles.filterCount}>
              showing <span>{filteredPosts.length}</span> of <span>{posts.length}</span>
            </span>
          </div>
                ...
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <div className={styles.categoryCount}>
                      <span>{category.count}</span> {t.articles}
                    </div>
                  </button>
                ))}
              </div>
